...
* `sr` and `games`: folders that contain eveything that the used simulator needs to be run
...
* `LICENSE.md`: markdown file that contains the licence of the used software
...
## System Issues
When the robot is perfectly aligned with a silver token that it has detected, due to the bulk of its flails, it never manages to get close to the token enough to grab it. This, however, it is not a script issue, but a simulator issue.  
