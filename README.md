...
* `sr`: a folder that contains eveything the simulator needs to be run
* `dox`: a folder that contains the documentation generated by the software doxygen based on the comments in the code
* `assignment1.py`: 

## How to run
The simulator used to see the effects f the python script running is named:   hon Robotic Simulator. This is a simple and portable robot simulator developed by [Student Robotics](https://studentrobotics.org).
...
## Script goal
Once run the run will appear an arena with several square blocks (either golden or silver) and a simulated robot in the upper-left corner. The goal of the script is to guide the robot along the path defined by the golden blocks, making sure that it doesn't collide with them. Furthermore the robot has to grab the silver blocks that it encounters along the way and move them behind itself.
